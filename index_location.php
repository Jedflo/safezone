<?php 
	...
	if (isset($_POST['save'])) {
		...
		fclose($file);
		$error = 'Successfully saved from Table Login to '.basename($fileName).".";
	}
	...
	if (isset($_POST['load'])){
		...
		foreach ($array as $row){
			$query = "INSERT INTO Login (UserID, UserName, email, Password, Name) 
			VALUES ('".$row["UserID"]."', '".$row["UserName"]."', '".$row["email"]."', '".$row["Password"]."', '".$row["Name"]."')";
			mysqli_query($con, $query);
		}
		mysqli_close($con);
		$error = 'Successfully loaded from '.basename($fileName)." to Table Login.";
	}
?>
